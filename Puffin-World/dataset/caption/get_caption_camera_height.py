...
def infer_one_image(model, processor, device: str, image_path: str, resize_to, max_new_tokens: int):
    ...
    messages = [
        {
            ...
            "content": [
                ...
                {"type": "text", "text": CAMERA_HEIGHT_PROMPT},
            ],
        }
    ]
...
def parse_args():
    p = argparse.ArgumentParser("Multi-GPU Qwen3-VL camera-height labeling")
    ...
    p.add_argument("--resize_w", type=int, default=640, help="Resize width; <=0 means no resize.")
    p.add_argument("--resize_h", type=int, default=320, help="Resize height; <=0 means no resize.")
    p.add_argument("--max_new_tokens", type=int, default=16)

    p.add_argument("--input_root", type=str, default="/data/kliao/data/360_dataset/")
    p.add_argument("--output_root", type=str, default="/data/kliao/data/Puffin-Pro/captions/camera_height/")
    p.add_argument("--folders", type=str, default="OmniPhotos",
                   help="Comma-separated folder names under input_root, e.g. 'OmniPhotos,Flickr360'. "
                        "Use 'ALL' to process every subfolder under input_root in sorted order.")
    return p.parse_args()
...
def main():
    ...
    if args.resize_w > 0 and args.resize_h > 0:
        ...
        print("[INFO] resize_to=None (keep original size)")
