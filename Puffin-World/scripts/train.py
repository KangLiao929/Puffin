...
# ---------------------------------------------------------------------------
# PyTorch >= 2.6 flipped torch.load's default to weights_only=True. That breaks
# RESUME: xtuner.get_seed_from_checkpoint / mmengine load a checkpoint whose meta
# contains an mmengine ConfigDict (not an allowlisted "safe global") -> raises
# UnpicklingError "Weights only load failed ... mmengine.config.config.ConfigDict".
# These are our OWN, trusted checkpoints, so restore the pre-2.6 default
# (weights_only=False) for every torch.load in this process.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Distributed robustness (set BEFORE any NCCL / process-group init). Raise the
# NCCL watchdog collective timeout 10min -> 30min so a transient AOSS/data or
# network stall does not kill the whole job. There is NO env var for the
# collective timeout, so inject it into deepspeed.init_distributed (which creates
# the process group). Flight-recorder env (TORCH_NCCL_TRACE_BUFFER_SIZE /
# TORCH_NCCL_DUMP_ON_TIMEOUT / ...) is set in the launch script, not here.
# ---------------------------------------------------------------------------
try:
    import datetime as _dt
    import deepspeed as _ds
    if not getattr(_ds.init_distributed, '_timeout_compat', False):
        _orig_ds_init = _ds.init_distributed

        def _ds_init_timeout(*args, **kwargs):
            kwargs.setdefault('timeout', _dt.timedelta(minutes=30))
            return _orig_ds_init(*args, **kwargs)

        _ds_init_timeout._timeout_compat = True
        _ds.init_distributed = _ds_init_timeout
except Exception:  # noqa: BLE001 -- deepspeed optional / signature drift; non-fatal
    pass
