...
def imread_cv2(path, options=cv2.IMREAD_COLOR):
    """Read an image or depthmap with OpenCV and return a numpy array.

    - .exr files are always read with IMREAD_ANYDEPTH (preserves float depth).
    - 3-channel reads are returned in RGB order (BGR->RGB conversion applied).
    - IMREAD_UNCHANGED preserves the raw dtype / channel layout (e.g. uint16
      depth PNGs stay as uint16 [H, W]).
    """
