...
class MegaDepth_Multi(BaseMultiViewDataset):
    ...
    def _load_data(self, split):
        ...
        megadepth_sets_path = file_client.download_file(osp.join(self.ROOT, "megadepth_sets_64.npz"), force_redownload=False)
        with np.load(megadepth_sets_path, allow_pickle=True) as data:
    ...
    def _get_views(self, idx, resolution, rng, num_views):
        ...
        for im_id in image_idxs:
            ...
            try:
                temp_image = file_client.download_file(osp.join(seq_path, img + ".jpg"), force_redownload=False)
                temp_depth = file_client.download_file(osp.join(seq_path, img + ".exr"), force_redownload=False)
                temp_cam = file_client.download_file(osp.join(seq_path, img + ".safetensor"), force_redownload=False)
                
                image = imread_cv2(temp_image)
                depthmap = imread_cv2(temp_depth)
