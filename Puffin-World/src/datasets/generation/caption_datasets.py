...
class CaptionDatasetGen(CaptionDataset):
    ...
    def _read_camera_pt(self, camera_file, cam_folder):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            image = self._read_image(data_sample['image'], self.image_folder)
            ...
            camera = self._read_camera(caption, image.height, image.width)
            ...
            if self.cap_folder_cot is not None:
                caption_cot = self._read_json(data_sample['annotation'].replace('train_scene_cam', 'train_thinking'), # replace the annotation
                                              self.cap_folder_cot)[self.cap_source].strip()
                caption_cot = (m.group(1).strip() if (m := re.search(r'<think>(.*?)</think>', caption_cot, re.S)) else caption_cot)
                caption = caption + ' ' + caption_cot
                #print(f"Caption with CoT: {caption}")  # Debug print to check the combined caption
            data = self._process_text(caption)
            
            # text-to-image generation (single-view) with camera map
            if self.data_type == 'cam2image':
            ...
            # image-to-image generation (cross-view)
                ...
                image_init = self._read_image(data_sample['image_init'], self.image_folder_init)
            ...
            print(f"Error when reading generation {self.data_path}:{self.data_list[idx]}: {e}", flush=True)
            raise e